...
const { calculateBenchmarkStats, getInflationRate } = require('../services/marketDataService');
...
const { getRiskAdjustedAllocation } = require('../services/strategyService');
...
router.post("/preview", requireAuth, async (req, res) => {
  try {
    ...
    // Get benchmarks using existing service
    const strategyService = require('../services/strategyService');
    const benchmarks = strategyService.generateDetailedBenchmarks(planningRegion);
    ...
    const aiSummary = `For a ${riskTolerance}-risk ${regionName} investor with ${savingsRate.toFixed(0)}% savings rate, we recommend ${allocation.equity}% equities, ${allocation.bonds}% bonds, targeting ${benchmarks.benchmarks.primary.cagr.toFixed(1)}% annual returns.`;
    
    res.json({
      status: 'success',
      allocation,
      aiSummary,
      expectedCagr: benchmarks.benchmarks.primary.cagr,
      benchmarks: {
        primary: benchmarks.benchmarks.primary.name,
        secondary: benchmarks.benchmarks.secondary.name,
        primaryCagr: benchmarks.benchmarks.primary.cagr,
        secondaryCagr: benchmarks.benchmarks.secondary.cagr
      },
