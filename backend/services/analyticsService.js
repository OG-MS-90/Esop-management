...
/**
 * Normalize ESOP mongoose docs into simple holdings structure
 */
function mapEsopToHoldings(esopDocs = []) {
  return esopDocs
    .map((doc) => {
      const ticker = doc.ticker;
      const vested = doc.vested || doc.quantity || 0;
      const exercisePrice = doc.exercisePrice || doc.price || 0;
      return { ticker, vested, exercisePrice };
    })
